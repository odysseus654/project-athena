...
AABox::AABox(const glm::dvec3& corner, double size) :
    _corner(corner), _scale(size, size, size) {
};

AABox::AABox(const glm::dvec3& corner, const glm::dvec3& dimensions) :
...
glm::dvec3 AABox::getVertex(BoxVertex vertex) const {
    switch (vertex) {
        case BOTTOM_LEFT_NEAR:
            return _corner + glm::dvec3(_scale.x, 0, 0);
        case BOTTOM_RIGHT_NEAR:
            return _corner;
        case TOP_RIGHT_NEAR:
            return _corner + glm::dvec3(0, _scale.y, 0);
        case TOP_LEFT_NEAR:
            return _corner + glm::dvec3(_scale.x, _scale.y, 0);
        case BOTTOM_LEFT_FAR:
            return _corner + glm::dvec3(_scale.x, 0, _scale.z);
        case BOTTOM_RIGHT_FAR:
            return _corner + glm::dvec3(0, 0, _scale.z);
        case TOP_RIGHT_FAR:
            return _corner + glm::dvec3(0, _scale.y, _scale.z);
        default: //quiet windows warnings
        case TOP_LEFT_FAR:
            return _corner + glm::dvec3(_scale);
    }
}

void AABox::setBox(const glm::dvec3& corner, float scale) {
    _corner = corner;
    _scale = glm::vec3(scale, scale, scale);
}

void AABox::setBox(const glm::dvec3& corner, const glm::vec3& scale) {
    _corner = corner;
    _scale = scale;
}

glm::dvec3 AABox::getFarthestVertex(const glm::vec3& normal) const {
    glm::dvec3 result = _corner;
    ...
    //    result.z += _scale.z;
    //}
    float blend = (float)(normal.x > 0.0);
    result.x += blend * _scale.x + (1.0 - blend) * 0.0;
    blend = (float)(normal.y > 0.0);
    result.y += blend * _scale.y + (1.0 - blend) * 0.0;
    blend = (float)(normal.z > 0.0);
    result.z += blend * _scale.z + (1.0 - blend) * 0.0;
    return result;
}

glm::dvec3 AABox::getNearestVertex(const glm::vec3& normal) const {
    glm::dvec3 result = _corner;
    // This is a branchless version of:
    //if (normal.x < 0.0f) {
    //    result.x += _scale.x;
...
bool AABox::contains(const AABox& otherBox) const {
    for (int v = BOTTOM_LEFT_NEAR; v < TOP_LEFT_FAR; v++) {
        glm::dvec3 vertex = otherBox.getVertex((BoxVertex)v);
        if (!contains(vertex)) {
            return false;
        }
    }
...
bool AABox::touches(const AABox& otherBox) const {
    glm::dvec3 relativeCenter = _corner - otherBox._corner + glm::dvec3((_scale - otherBox._scale) * 0.5f);
...
bool AABox::contains(const AACube& otherCube) const {
    for (int v = BOTTOM_LEFT_NEAR; v < TOP_LEFT_FAR; v++) {
        glm::dvec3 vertex = otherCube.getVertex((BoxVertex)v);
        if (!contains(vertex)) {
            return false;
        }
    }
...
bool AABox::touches(const AACube& otherCube) const {
    glm::dvec3 relativeCenter = _corner - otherCube.getCorner() + ((glm::dvec3(_scale) - otherCube.getDimensions()) * 0.5 );

    glm::dvec3 totalHalfScale = (glm::dvec3(_scale) + otherCube.getDimensions()) * 0.5;
...
bool AABox::expandedContains(const glm::dvec3& point, double expansion) const {
...
bool AABox::expandedIntersectsSegment(const glm::dvec3& start, const glm::dvec3& end, double expansion) const {
    ...
    // check each axis
    glm::dvec3 expandedCorner = _corner - glm::dvec3(expansion, expansion, expansion);
    glm::dvec3 expandedSize = glm::dvec3(_scale) + glm::dvec3(expansion, expansion, expansion) * 2.0;
...
bool AABox::findRayIntersection(const glm::dvec3& origin, const glm::vec3& direction, const glm::vec3& invDirection,
                                double& distance, BoxFace& face, glm::vec3& surfaceNormal) const {
    return findRayAABoxIntersection(origin, direction, invDirection, _corner, _scale, distance, face, surfaceNormal);
}

bool AABox::findParabolaIntersection(const glm::dvec3& origin, const glm::vec3& velocity, const glm::vec3& acceleration,
...
bool AABox::touchesSphere(const glm::dvec3& center, double radius) const {
    // Avro's algorithm from this paper: http://www.mrtc.mdh.se/projects/3Dgraphics/paperF.pdf
    glm::dvec3 e = glm::max(_corner - center, Vectors::ZERO) + glm::max(center - _corner - glm::dvec3(_scale), Vectors::ZERO);
    return glm::length2(e) <= radius * radius;
}

bool AABox::findSpherePenetration(const glm::dvec3& center, double radius, glm::vec3& penetration) const {
    glm::vec4 center4 = glm::vec4(center, 1.0f);

    float minPenetrationLength = FLT_MAX;
    for (int i = 0; i < FACE_COUNT; i++) {
        glm::vec4 facePlane = getPlane((BoxFace)i);
        glm::dvec3 vector = getClosestPointOnFace(center, (BoxFace)i) - center;
        ...
        if (vectorLength < minPenetrationLength) {
            ...
            penetration = (vectorLength < EPSILON) ? glm::dvec3(-facePlane) * radius :
...
bool AABox::findCapsulePenetration(const glm::dvec3& start, const glm::dvec3& end, double radius, glm::vec3& penetration) const {
    glm::dvec4 start4 = glm::dvec4(start, 1.0);
    glm::dvec4 end4 = glm::dvec4(end, 1.0);
    ...
    for (int i = 0; i < FACE_COUNT; i++) {
        ...
        glm::dvec4 facePlane = getPlane((BoxFace)i);
        glm::dvec3 closest = (glm::dot(start4, facePlane) <= glm::dot(end4, facePlane)) ?
            getClosestPointOnFace(start4, startToEnd, (BoxFace)i) : getClosestPointOnFace(end4, -startToEnd, (BoxFace)i);
        glm::dvec3 vector = -computeVectorFromPointToSegment(closest, start, end);
        if (glm::dot(vector, glm::dvec3(facePlane)) < 0.0) {
            // outside this face, so use vector to closest point to determine penetration
            return ::findSpherePenetration(vector, glm::vec3(-facePlane), radius, penetration);
        }
        float vectorLength = glm::length(vector);
...
glm::dvec3 AABox::getClosestPointOnFace(const glm::dvec3& point, BoxFace face) const {
    switch (face) {
        case MIN_X_FACE:
            return glm::clamp(point, glm::dvec3(_corner.x, _corner.y, _corner.z),
                glm::dvec3(_corner.x, _corner.y + _scale.y, _corner.z + _scale.z));

        case MAX_X_FACE:
            return glm::clamp(point, glm::dvec3(_corner.x + _scale.x, _corner.y, _corner.z),
                glm::dvec3(_corner.x + _scale.x, _corner.y + _scale.y, _corner.z + _scale.z));

        case MIN_Y_FACE:
            return glm::clamp(point, glm::dvec3(_corner.x, _corner.y, _corner.z),
                glm::dvec3(_corner.x + _scale.x, _corner.y, _corner.z + _scale.z));

        case MAX_Y_FACE:
            return glm::clamp(point, glm::dvec3(_corner.x, _corner.y + _scale.y, _corner.z),
                glm::dvec3(_corner.x + _scale.x, _corner.y + _scale.y, _corner.z + _scale.z));

        case MIN_Z_FACE:
            return glm::clamp(point, glm::dvec3(_corner.x, _corner.y, _corner.z),
                glm::dvec3(_corner.x + _scale.x, _corner.y + _scale.y, _corner.z));

        default: //quiet windows warnings
        case MAX_Z_FACE:
            return glm::clamp(point, glm::dvec3(_corner.x, _corner.y, _corner.z + _scale.z),
                glm::dvec3(_corner.x + _scale.x, _corner.y + _scale.y, _corner.z + _scale.z));
    }
}

glm::dvec3 AABox::getClosestPointOnFace(const glm::dvec4& origin, const glm::vec4& direction, BoxFace face) const {
    ...
    for (int i = 0; i < FACE_COUNT; i++) {
        ...
        return getClosestPointOnFace(glm::dvec3(intersection), face);
    ...
    if (anyOutside) {
        ...
        if (minDistance != FLT_MAX) {
            return getClosestPointOnFace(glm::dvec3(origin + glm::dvec3(direction) * minDistance), face);
        }
    }

    // last resort or all inside: clamp origin to face
    return getClosestPointOnFace(glm::dvec3(origin), face);
}

bool AABox::touchesAAEllipsoid(const glm::dvec3& center, const glm::dvec3& radials) const {
    ...
    for (int i = 0; i < FACE_COUNT; i++) {
        glm::dvec3 closest = getClosestPointOnFace(center, (BoxFace)i) - center;
        float x = closest.x;
        float y = closest.y;
        float z = closest.z;
        float a = radials.x;
...
AABox AABox::clamp(const glm::dvec3& min, const glm::dvec3& max) const {
    glm::dvec3 clampedCorner = glm::clamp(_corner, min, max);
    glm::dvec3 clampedTopFarLeft = glm::clamp(calcTopFarLeft(), min, max);
    glm::dvec3 clampedScale = clampedTopFarLeft - clampedCorner;

    return AABox(clampedCorner, clampedScale);
}

AABox AABox::clamp(double min, double max) const {
    glm::dvec3 clampedCorner = glm::clamp(_corner, min, max);
    glm::dvec3 clampedTopFarLeft = glm::clamp(calcTopFarLeft(), min, max);
    glm::dvec3 clampedScale = clampedTopFarLeft - clampedCorner;
...
void AABox::rotate(const glm::quat& rotation) {
    ...
    auto maximum = _corner + glm::dvec3(_scale);
...
void AABox::transform(const Transform& transform) {
    scale(glm::vec3(transform.getScale()));
    rotate(transform.getRotation());
    translate(glm::vec3(transform.getTranslation()));
}
...
void AABox::transform(const glm::mat4& matrix) {
    ...
    auto center = _corner + glm::dvec3(halfSize);
