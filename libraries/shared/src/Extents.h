...
public:
    Extents() { }
    Extents(const glm::dvec3& minimum, const glm::dvec3& maximum) : minimum(minimum), maximum(maximum) {}
    ...
    /// \return true if point is within current limits
    void shiftBy(const glm::dvec3& delta) { minimum += delta; maximum += delta; }
    ...
    glm::dvec3 size() const { return maximum - minimum; }
    double largestDimension() const { return glm::compMax(size()); }

    /// \return new Extents which is original rotated around origin by rotation
    Extents getRotated(const glm::dquat& rotation) const {
        Extents temp(minimum, maximum);
        temp.rotate(rotation);
        return temp;
    }

    glm::dvec3 minimum{ Vectors::MAX };
    glm::dvec3 maximum{ Vectors::MIN };
};
